...
import os, sys, math
from PIL import Image, ImageTk, ImageGrab, ImageFilter, ImageDraw, ImageFont
...
BG2      = "#2b2d30"   # 工具栏背景
BG3      = "#3c3f41"   # 按钮默认
ACCENT   = "#4f8ef7"   # 蓝色强调（贴图）
GREEN    = "#3dba78"   # 绿色（保存/选中）
RED      = "#e05c5c"   # 红色预设
BORDER   = "#444"
FG       = "#e0e0e0"
FG_DIM   = "#888"
...
class SelectionOverlay:
    def __init__(self, on_selected):
        ...
        # 背景图层（只有这一个 image item，拖拽时替换它）
        self.bg_photo = ImageTk.PhotoImage(self.dim_img)
        self.bg_item = self.canvas.create_image(
            0, 0, anchor="nw", image=self.bg_photo)

        # 顶部提示栏（固定，始终在最上层）
        self.canvas.create_rectangle(0, 0, self.screen_w, 44,
                                     fill="#1e1f22", outline="",
                                     tags="ui")
        self.canvas.create_text(self.screen_w // 2, 22,
                                text="拖拽选择截图区域    |    ESC 取消",
                                fill="#e0e0e0", font=("微软雅黑", 13),
                                tags="ui")

        # 选区边框和尺寸标注（统一用 tag 管理）
        self.rect_id  = None
        self.size_ids = []
    ...
    def on_drag(self, e):
        ...
        draw.rectangle([tx-2, ty-2, tx + len(label)*7, ty+16],
                       fill="#1e1f22")
...
class AnnotationEditor:
    ...
    def _build_toolbar(self):
        ...
        tk.Label(bar, text="✂ SnapTool", bg=BG2, fg=ACCENT,
                 font=("微软雅黑", 11, "bold"), padx=14).pack(side="left")
        ...
        sep2.pack(side="left", fill="y", pady=6, padx=6)

        # 颜色预设
        for c in [RED, "#f0a500", GREEN, ACCENT, "#ffffff"]:
            dot = tk.Label(bar, bg=c, width=2, cursor="hand2",
                           relief="flat", pady=10)
        ...
        self.color_preview = tk.Label(bar, bg=self.color, text=" 自定义 ",
                                      fg="white", font=FONT,
                                      cursor="hand2", pady=5, padx=6)
        self.color_preview.pack(side="left", padx=(4, 8))
        self.color_preview.bind("<Button-1>", lambda e: self.pick_color())

        sep3 = tk.Frame(bar, bg=BORDER, width=1)
        sep3.pack(side="left", fill="y", pady=6, padx=4)

        # 线宽
        tk.Label(bar, text="线宽", bg=BG2, fg=FG_DIM, font=FONT,
        ...
        sep4 = tk.Frame(bar, bg=BORDER, width=1)
        sep4.pack(side="left", fill="y", pady=6, padx=6)
    ...
    def _build_canvas(self):
        ...
        self.canvas = tk.Canvas(inner, bg="#111",
                                highlightthickness=0,
        ...
        self.status_var = tk.StringVar(value=f"图片尺寸：{img_w} × {img_h}  |  F1 新截图  |  Ctrl+Z 撤销  |  Ctrl+S 保存")
        tk.Label(status, textvariable=self.status_var,
                 bg=BG2, fg=FG_DIM, font=("微软雅黑", 9),
    ...
    def _ask_text(self):
        ...
        tk.Label(dlg, text="文字内容", bg=BG, fg=FG,
                 font=FONT).grid(row=0, column=0, padx=12, pady=(14,4), sticky="w")
        entry = tk.Entry(dlg, font=("微软雅黑", 12), bg=BG3, fg=FG,
                         insertbackground=FG, relief="flat",
                         width=24, bd=6)
        entry.grid(row=1, column=0, columnspan=2, padx=12, pady=(0,10), sticky="ew")
        entry.focus_set()

        tk.Label(dlg, text="字号", bg=BG, fg=FG,
                 font=FONT).grid(row=2, column=0, padx=12, pady=(0,4), sticky="w")
        ...
        for s in [14, 18, 24, 32, 48]:
            rb = tk.Radiobutton(size_frame, text=str(s), variable=size_var,
                                value=s, bg=BG, fg=FG,
                                selectcolor=BG, activebackground=BG,
                                activeforeground=ACCENT,
                                font=("微软雅黑", 10), indicatoron=False,
        ...
        make_round_btn(btn_row, "取消", BG3,    FG,      cancel,  padx=20, pady=5).pack(side="left", padx=6)
    ...
    def on_drag(self, e):
        ...
        if self.tool == "rect":
            ...
            if len(self.pen_points) >= 2:
                # 只画最后一段，不重建整条线
                p1 = self.pen_points[-2]
                p2 = self.pen_points[-1]
                self.preview_ids.append(self.canvas.create_line(
                    p1[0], p1[1], p2[0], p2[1],
    ...
    def on_release(self, e):
        ...
        img_w, img_h = self.base.size
        self.status_var.set(
            f"图片尺寸：{img_w} × {img_h}  |  F1 新截图  |  Ctrl+Z 撤销  |  Ctrl+S 保存")
    ...
    def _redraw(self):
        ...
        for ann in self.annotations:
            ...
            if t == "rect":
                ...
                try:
                    font = ImageFont.truetype("msyh.ttc", ann.get("size", 24))
                except Exception:
                    font = ImageFont.load_default()
                draw.text((ann["x"], ann["y"]), ann["text"],
                          fill=ann["color"], font=font)
            elif t == "blur":
                region  = img.crop((ann["x1"], ann["y1"],
                                    ann["x2"], ann["y2"]))
    ...
    def copy_to_clipboard(self):
        ...
        try:
            if sys.platform == "win32":
                ...
                out = BytesIO()
                self.base.convert("RGB").save(out, "BMP")
                ...
                # Mac：保存临时 PNG 再用 osascript 写入剪贴板
...
class SnapToolApp:
    ...
    def _start_tray(self):
        ...
        try:
            ...
            # 生成一个简单的托盘图标（绿色圆形+剪刀）
            ico = PILImage.new("RGBA", (64, 64), (0, 0, 0, 0))
            d = PILDraw.Draw(ico)
            d.ellipse([4, 4, 60, 60], fill="#4f8ef7")
            d.text((18, 16), "✂", fill="white")

            menu = pystray.Menu(
                pystray.MenuItem("显示帮助", lambda: self.root.after(0, self._show_hint)),
                pystray.MenuItem("退出",    lambda: self.root.after(0, self.quit)),
            )
            self._tray = pystray.Icon("SnapTool", ico, "SnapTool 截图工具", menu)

            t = threading.Thread(target=self._tray.run, daemon=True)
            t.start()
        except ImportError:
            pass  # 没装 pystray 就跳过，不影响主功能
    ...
    def _show_hint(self):
        ...
        for i, (k, v) in enumerate(rows):
            row = tk.Frame(shortcuts, bg=BG2 if i%2==0 else BG,
                           pady=7, padx=12)
            row.pack(fill="x")
            tk.Label(row, text=k, bg=row["bg"], fg=ACCENT,
                     font=("Consolas", 10, "bold"), width=12,
                     anchor="w").pack(side="left")
            tk.Label(row, text=v, bg=row["bg"], fg=FG,
    ...
    def _start_hotkeys(self):
        ...
        def hotkey_thread():
            ...
            VK_F1     = 0x70
            HOTKEY_ID = 1
            ctypes.windll.user32.RegisterHotKey(None, HOTKEY_ID, 0, VK_F1)
